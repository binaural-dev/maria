...
class ReportAccountFinancialReport(models.Model):
    ...
    def _get_options(self, previous_options=None):
        ...
        res.update({'currency': [{'name': alternate_currency.name, 'id': alternate_currency.id, 'selected': False},
                                 {'name': self.env.user.company_id.currency_id.name, 'id': self.env.user.company_id.id,
                                  'selected': True}]})
        return res
...
#                     %s AS period_index,
#                     COALESCE(SUM(ROUND(%s * account_move_line.balance * account_move_line.foreign_currency_rate, currency_table.precision)), 0.0) AS balance
#                 FROM ''' + tables + '''
