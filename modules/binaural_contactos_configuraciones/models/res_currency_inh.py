...
class ResCurrencyInh(models.Model):
    ...
    def _convert(self, from_amount, to_currency, company, date, round=True, byrate=1):
        ...
        name_foreign_currency = to_currency.name
        name_base_currency = 'USD' if name_foreign_currency == 'VEF' else 'VEF'

        if self == to_currency:
            to_amount = from_amount
        elif byrate != 1:
            to_amount = from_amount * decimal_function.getCurrencyValue(
                rate=byrate, base_currency=name_base_currency, foreign_currency=name_foreign_currency)
        # apply rounding
